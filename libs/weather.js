...
async function getTonightWeather() {
  ...
  const res = await axios.get(`https://www.cwb.gov.tw/V7/forecast/f_index.htm?_=${time}`);
...
async function getRegionWeather(regionName) {
  const res = await axios.get(`https://www.cwb.gov.tw/V7/forecast/taiwan/${regionName}.htm`);
  const { document } = (new JSDOM(res.data)).window;
  const intervalNodes = document.querySelectorAll('#box8 > table > tbody > tr');
  const tonightNode = [...intervalNodes].find((e) => e.querySelector('th').textContent.includes('今晚至明晨'));
  return {
